...
func _run() -> void:
	...
	test.check(light.light_projector == null and light.get_meta("peraviz_gobo_texture", null) == null, "An open slot must leave normal unmasked spot lighting")
	...
	for mode in [0, 1, 2, 0]:
		...
		var fog_count: int = light.find_children("PeravizFogVolumeGoboBeam", "FogVolume", false).size()
		if mode == 0:
