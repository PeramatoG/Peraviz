...
func _run() -> void:
	...
	test.check(loader.renderer.get_last_parameter_write_count() == 1, "A visible volumetric dynamic update should write one packed instance parameter")
	...
	service.apply_emitter_intensity(loader, "fixture", 201, 2, 1.0, 25.0, 0.0, 25.0, 5.0)
