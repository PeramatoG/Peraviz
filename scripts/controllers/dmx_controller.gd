...
func _report_performance_trace_if_needed() -> void:
	...
	print("[peraviz-presentation] mode=%d vector=%d fog_allocated=%d fog_visible=%d fog_creations=%d fog_writes=%d shadow_masks=%d projectors=%d realtime_spotlights=%d shadow_spotlights=%d gobo_beams=%d open_beams=%d" % [int(counters.get("beam_presentation", 1)), int(counters.get("active_vector_prisms", 0)), int(counters.get("allocated_fog_volumes", 0)), int(counters.get("active_fog_volumes", 0)), int(counter_delta.get("fog_volume_creations", 0)), int(counter_delta.get("fog_parameter_writes", 0)), int(counters.get("active_shadow_masks", 0)), int(counters.get("active_light_projectors", 0)), int(counters.get("spotlight_visible_count", 0)), int(counters.get("shadow_enabled_spotlights", 0)), int(counters.get("gobo_visible_beams", 0)), int(counters.get("open_visible_beams", 0))])
