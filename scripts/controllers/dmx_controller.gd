...
func _report_performance_trace_if_needed() -> void:
	...
	print("[peraviz-presentation] mode=%d vector=%d fog_volumes=%d shadow_masks=%d projectors=%d realtime_spotlights=%d shadow_spotlights=%d gobo_beams=%d open_beams=%d" % [int(counters.get("beam_presentation", 1)), int(counters.get("active_vector_prisms", 0)), int(counters.get("active_fog_volumes", 0)), int(counters.get("active_shadow_masks", 0)), int(counters.get("active_light_projectors", 0)), int(counters.get("spotlight_visible_count", 0)), int(counters.get("shadow_enabled_spotlights", 0)), int(counters.get("gobo_visible_beams", 0)), int(counters.get("open_visible_beams", 0))])
