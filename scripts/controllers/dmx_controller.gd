...
func _report_performance_trace_if_needed() -> void:
	...
	print("[peraviz-perf] mode=%s fps=%.1f process_ms=%.2f draws=%d objects=%d primitives=%d nodes=%d resources=%d packets=%d relevant=%d irrelevant=%d rejects=%d overwrites=%d pump_hz=%d states=%d rx_native_us=%d/%d/%d native_apply_us=%d/%d/%d rx_apply_us=%d/%d/%d frames=%d rows=%d applied=%d noop=%d failed=%d domains=%s domain_us=%s outputs=%d beams_considered=%d light_props=%d light_visibility=%d beam_params=%d beam_visibility=%d beam_topology=%d material_params=%d gobo_topology=%d gobo_params=%d visible_beams=%d visible_lights=%d apply_max_us=%d" % [_render_diagnostic_mode, Performance.get_monitor(Performance.TIME_FPS), Performance.get_monitor(Performance.TIME_PROCESS) * 1000.0, int(Performance.get_monitor(Performance.RENDER_TOTAL_DRAW_CALLS_IN_FRAME)), int(Performance.get_monitor(Performance.RENDER_TOTAL_OBJECTS_IN_FRAME)), int(Performance.get_monitor(Performance.RENDER_TOTAL_PRIMITIVES_IN_FRAME)), int(Performance.get_monitor(Performance.OBJECT_NODE_COUNT)), int(Performance.get_monitor(Performance.OBJECT_RESOURCE_COUNT)), accepted_delta, relevant_delta, _stat_delta(stats, "irrelevant_packets"), _stat_delta(stats, "out_of_order_dropped"), _stat_delta(stats, "mailbox_overwrites"), _production_pump_calls, int(_perf_interval.get("states", 0)), int(stats.get("rx_to_native_p50_us", 0)), int(stats.get("rx_to_native_p95_us", 0)), int(stats.get("rx_to_native_max_us", 0)), int(stats.get("native_to_apply_p50_us", 0)), int(stats.get("native_to_apply_p95_us", 0)), int(stats.get("native_to_apply_max_us", 0)), int(stats.get("rx_to_apply_p50_us", 0)), int(stats.get("rx_to_apply_p95_us", 0)), int(stats.get("rx_to_apply_max_us", 0)), int(_perf_interval.get("frames", 0)), int(_perf_interval.get("rows", 0)), int(_perf_interval.get("applied", 0)), int(_perf_interval.get("noop", 0)), int(_perf_interval.get("failed", 0)), str(_perf_interval.get("domains", {})), str(_perf_interval.get("domain_us", {})), int(counter_delta.get("physical_outputs_considered", 0)), int(counter_delta.get("beams_considered", 0)), int(counter_delta.get("light_properties_written", 0)), int(counter_delta.get("light_visibility_calls", 0)), int(counter_delta.get("beam_shader_parameters_written", 0)), int(counter_delta.get("beam_visibility_transitions", 0)), int(counter_delta.get("beam_topology_rebuilds", 0)), int(counter_delta.get("material_parameters_written", 0)), int(counter_delta.get("gobo_topology_updates", 0)), int(counter_delta.get("gobo_parametric_updates", 0)), int(counters.get("beam_visible_count", 0)), int(counters.get("spotlight_visible_count", 0)), _worst_dmx_tick_usec])
