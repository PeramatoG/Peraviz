...
func _report_performance_trace_if_needed() -> void:
	...
	print("[peraviz-presentation] mode=%d shared_haze=%d shared_haze_density=%.4f per_emitter_fog_volumes=%d native_dimmer_targets=%d native_output_records=%d unique_native_emitters=%d active_output_records=%d active_native_emitters=%d cached_fixture_lights=%d vector_resources=%d surface_projectors=%d shadow_masks=%d realtime_spotlights=%d shadow_spotlights=%d gobo_outputs=%d open_outputs=%d" % [int(counters.get("beam_presentation", 1)), 1 if bool(counters.get("shared_haze", false)) else 0, float(counters.get("shared_haze_density", 0.0)), int(counters.get("allocated_fog_volumes", 0)), int(counters.get("native_dimmer_targets", 0)), int(counters.get("native_output_records", 0)), int(counters.get("unique_native_emitters", 0)), int(counters.get("active_output_records", 0)), int(counters.get("active_native_emitters", 0)), int(counters.get("cached_fixture_lights", 0)), int(counters.get("active_vector_prisms", 0)), int(counters.get("active_light_projectors", 0)), int(counters.get("active_shadow_masks", 0)), int(counters.get("spotlight_visible_count", 0)), int(counters.get("shadow_enabled_spotlights", 0)), int(counters.get("gobo_visible_beams", 0)), int(counters.get("open_visible_beams", 0))])
