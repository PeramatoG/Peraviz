...
func update_for_light(light: SpotLight3D, beam_params: Dictionary, gobo_texture: Texture2D, visual_settings: Dictionary) -> void:
	...
	var presentation_intensity_max: float = min(max(float(beam_params.get("intensity_max", 100.0)), 0.01), 20.0)
	fog_material.set_shader_parameter("intensity", clamp(scaled_intensity / presentation_intensity_max, 0.0, 1.0))
