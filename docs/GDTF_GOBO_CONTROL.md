...
This is a transitional bridge: static wheel slot data and cached GDTF texture metadata are cached per fixture by the DMX runtime, while the live numeric channel state comes from the native fixed-stride frame. The initial beam setup path skips the legacy gobo resolver after a live visual-frame gobo has been applied so the projector texture metadata survives first-use beam initialization, and beam materials enable their gobo texture sampler whenever a live gobo texture is present. The intended next step is to move the remaining slot/range and motion-mode resolution into a compact native gobo runtime resolver so Godot receives a render-ready gobo state instead of consulting cached capability dictionaries.
