...
- Reduced live DMX apply stalls by using native render-ready DMX values, avoiding unnecessary gobo control construction when gobo capabilities are unchanged, reusing shared beam parameter templates for fixtures with matching beam state, and keeping the main-thread apply drain from holding the runtime mutex.
