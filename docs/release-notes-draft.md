...
- Added the first native BeamOptics foundation: setup-time Beam profiles, native Zoom ChannelFunction compilation, target-oriented BeamOptics rows, cached optics targets, official/model/render aperture-radius diagnostics, and parametric Lightweight Prism near/far spread updates, corrected lens-side/far-end axial radius mapping, and packed shader instance parameters to stay within Godot renderer limits, kept explicit BeamRadius as the preferred GDTF near-aperture source, and scaled imported model assets to declared GDTF Model dimensions before lens measurement.
