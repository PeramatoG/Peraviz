...
- Native color-wheel composition now keeps one authoritative final Beam color: multiple seated or indexed wheel layers accumulate in C++ with the target's base color, Godot treats `WheelSelection` as metadata only, and each dirty snapshot emits at most one final `EmitterColor` row per Beam.
