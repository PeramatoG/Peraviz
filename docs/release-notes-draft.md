...
- Improved live DMX responsiveness with a general visual attribute-mask batch path that initializes persistent fixture visual state from the first valid DMX row, applies dimmer, pan/tilt, color, zoom, strobe, beam, light, emissive, and material updates only to their dependent render state, rebuilds missing visual handles, coalesces pending rows by fixture with latest-state-wins, and exposes copyable Technical Monitor JSON metrics for receiver, worker, apply, backlog, frame-budget, attempted/applied writes, and per-attribute counters.
