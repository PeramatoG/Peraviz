...
- Reduced live DMX apply stalls by using a compact native decoder payload, avoiding unnecessary gobo control construction when gobo capabilities are unchanged, reusing cached beam defaults, and keeping the main-thread apply drain from holding the runtime mutex.
