...
## GDTF Wheel and Attribute Inspector boundary

Checkpoint 08E3 keeps wheel, filter, ColorCIE, archive-resource, and DMX inspection logic in native non-GUI services. Presentation code may consume typed read-only results for slider display, row highlighting, resource previews, and WheelSlot galleries, but it must not parse XML, write XML, mutate sessions, mark projects dirty, or send live DMX from the inspector path.
