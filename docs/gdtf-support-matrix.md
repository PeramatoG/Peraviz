...
| Color(n) seated color wheel | WheelSelection | partial | Parser/compiler/runtime now support a verified seated discrete color-wheel slice: ordered wheel slots, exact ChannelFunction.Wheel links, exact ChannelSet.WheelSlotIndex, cooked ColorCIE/filter slot optics, exact Beam target bindings, native DMX evaluation, sparse WheelSelection rows, and target-local Godot application to SpotLight, beam and lens resources. Indexed spatial split, spin, random and Audio remain unsupported/deferred. |
