...
| Color(n) seated color wheel | WheelSelection | partial | Parser/compiler/runtime now support a verified seated discrete color-wheel slice: ordered wheel slots, exact ChannelFunction.Wheel links, exact ChannelSet.WheelSlotIndex, standard DMXFrom-only effective ranges, cooked ColorCIE/filter slot optics with bounded transmission shape, scalar gain applied once, and linear composition, exact Beam target bindings, native DMX evaluation, sparse WheelSelection rows, and target-local Godot application to SpotLight, beam and lens resources. WheelIndex currently uses a documented non-spatial aggregate fallback. Multiple wheel layers on the same Beam compose natively into one final EmitterColor row while WheelSelection remains metadata; final spatial split, spin, random and Audio remain unsupported/deferred. |
| Gobo(n), AnimationWheel(n), ColorMacro(n), Prism(n) | WheelSelection / WheelMotion | planned | Renderer appliers remain for future native wheel-family work, but runtime compiler support is not claimed for these domains. |
