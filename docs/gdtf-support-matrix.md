...
| Gobo motion | Partial | Exact `Gobo(n)Pos` physical Angle and ModeMaster activation are evaluated natively and applied parametrically to one visible selected-gobo layer per Beam/wheel instance without topology work or `PlacementOffset`. Independently moving multi-layer composition, PosRotate, spin, shake, random, audio, and effects remain deferred. |
