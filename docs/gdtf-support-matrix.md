...
| Color(n) wheel groundwork | WheelSelection / WheelMotion | partial | Protocol v2.1 now defines native renderer-ready optical and sparse motion rows for color-wheel selection, index/split, spin synchronization and deterministic random scheduling. Full parser/compiler binding coverage remains in progress; gobo and animation wheels stay outside this support claim. |
