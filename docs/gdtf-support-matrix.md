...
The renderer keeps official optical radius separate from measured model aperture and selected visual near radius. Explicit BeamRadius is the preferred GDTF source for the render near radius; the Lightweight Prism path also records measured aperture radius, selected render near radius, selection source, and mismatch ratio so unit/scale mismatches are diagnosable instead of silently hidden. Imported model assets are scaled to the declared GDTF Model Length/Width/Height before lens measurement so BeamRadius and the visible lens model share the same meter space.
