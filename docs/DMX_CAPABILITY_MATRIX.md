...
| Gobo | Partial | Partial | Partial | Native and headless tests cover exact seated `Gobo(n)` slot/asset rows, open clearing, normalized topology reuse, and cached static binary multi-wheel composition. | Position, rotation, shake, independently moving composition, and surface projection remain unsupported; legacy compatibility metadata is not authoritative for seated live selection. |
