...
The production vertical slice currently supports seated discrete `Color(n)` selection with standard DMXFrom-only ChannelSet ranges inferred from the next ChannelSet and parent ChannelFunction range. The parser records ordered one-based wheel slots and exact `ChannelFunction.Wheel` / `ChannelSet.WheelSlotIndex` metadata. The runtime scene compiler cooks immutable palette slots, binds the color-wheel source to exact Beam render-target IDs, serializes palettes and bindings through the packed scene bridge, and installs only valid bindings as universe interests. Runtime DMX changes update one persistent layer per physical `(beam_render_target_id, wheel_renderer_id)` in the Beam target's ordered wheel stack, emit sparse `WheelSelection` metadata for changed physical wheels, and recompose the exact Beam once into one final `EmitterColor` row when the uniform output changes. Multiple ChannelFunctions that control the same wheel, such as `Color(n)` Select and `Color(n)WheelIndex`, are mutually exclusive ranges and replace the same layer instead of multiplying together. Godot resolves the exact Beam output record for `EmitterColor` and stores wheel metadata for future spatial split rendering without applying it as a second color authority.
