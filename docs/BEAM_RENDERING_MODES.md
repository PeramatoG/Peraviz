...
### Core/field appearance resources

Lightweight and Volumetric projected beams now allocate a cached field resource and a cached core resource for each Beam target. The added resource provides actual interior radial samples while preserving the existing beam origin, direction, aperture, visual length, Zoom, and gobo orientation. Appearance changes update material parameters; Dimmer and Zoom do not create new resources.
